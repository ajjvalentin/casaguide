"""Activités du secteur (V2-71) — CLAUDE + recherche web.

Tests PURS (aucune base, aucun réseau) du cœur `claude_enrich.fetch_activities`
(« que fait-on ici ? » : surf, randonnée, plongée…) et du rendu SSR
`guide_page._fact_activities` (encart standalone de l'onglet « Autour »).
...
def test_ssr_empty_activities_renders_nothing():
    assert guide_page._fact_activities({"activities": []}, "fr") == ""
    assert guide_page._fact_activities({}, "fr") == ""


def test_activities_title_localised():
    content = {"activities": [{"activity": "Surf", "where": "beach",
                               "source_url": "https://x"}]}
    assert "Activities in the area" in guide_page._fact_activities(content, "en")
