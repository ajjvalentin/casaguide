/* Compensation des ancres natives sous la barre d'onglets collante (V2-07 volet
   1quater).

   Pilote le VRAI /frontend/guide/app.js dans Chrome headless (harnais
   guide-anchor-offset-harness.html) avec la vraie guide.css : un tap sur la tuile
   de service « Livraison de repas » (ancre native #E_food_delivery) vise la carte
   de section, laquelle porte scroll-margin-top:64px (style CALCULÉ) → elle se
   décolle de la barre d'onglets collante ; les autres cibles d'ancre (.svc-grid,
   .cat) gardent leur compensation. Verdict dans le DOM dumpé (test ignoré si
   aucun Chrome).
...
test("compensation d'ancre V2-07 1quater : la tuile vise la section, scroll-margin-top:64px, pas de régression des autres ancres", async (t) => {
  const chrome = requireChrome("guide-anchor-offset.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "guide-anchor-offset-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("guide-anchor-offset-harness.html", verdict);
  } finally {
    server.close();
  }
});
