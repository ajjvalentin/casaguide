/* Bloc « Photo de couverture » (V2-30).

   Pilote le VRAI /frontend/js/components/cover.js dans Chrome headless (harnais
...
test("bloc « Photo de couverture » : repli, choix, téléversement sans section, retrait", async (t) => {
  const chrome = requireChrome("cover.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "cover-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("cover-harness.html", verdict);
  } finally {
    server.close();
  }
});
