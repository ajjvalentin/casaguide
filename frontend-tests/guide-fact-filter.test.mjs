/* Mode filtré des tuiles de fait de zone (V2-07 volet 1quinquies).

   Pilote le VRAI /frontend/guide/app.js + search.js dans Chrome headless (harnais
   guide-fact-filter-harness.html) avec la vraie guide.css : la tuile « Livraison de
   repas » ouvre le MÊME mode filtré que les tuiles de catégorie (seule la section
   E_food_delivery visible + « Retour aux services », carte masquée), le retour
   restaure la grille, une tuile de catégorie reste inchangée (non-régression
...
test("mode filtré fait de zone V2-07 1quinquies : tuile Livraison → section seule + retour + non-régression catégorie + recherche + deep-link", async (t) => {
  const chrome = requireChrome("guide-fact-filter.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "guide-fact-filter-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("guide-fact-filter-harness.html", verdict);
  } finally {
    server.close();
  }
});
