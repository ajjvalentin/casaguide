/* Séparation stricte des contextes de l'éditeur (V2-26b).

   Pilote le VRAI /frontend/js/views/editor.js dans Chrome headless (harnais
   editor-context-harness.html) : contexte voyageur (barre latérale = chapitres
   du guide, sans groupe staff), contexte staff (barre latérale = cahier d'équipe
   + bloc de transmission, en-tête « Cahier de préparation », actions guide
...
test("V2-26b : contextes voyageur/staff strictement séparés, bascule gatée, jauges", async (t) => {
  const chrome = requireChrome("editor-context.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "editor-context-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("editor-context-harness.html", verdict);
  } finally {
    server.close();
  }
});
