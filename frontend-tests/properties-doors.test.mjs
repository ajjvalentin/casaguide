/* Double porte de la carte du logement (V2-26).

   Pilote le VRAI /frontend/js/views/properties.js dans Chrome headless (harnais
   properties-doors-harness.html) : deux portes rendues, porte Guide → éditeur
   voyageur, porte Équipe → éditeur staff, gating Solo (badge Pro + encart au clic,
...
test("double porte V2-26 : deux portes, gating staff (Pro/Aperçu), lien + QR équipe", async (t) => {
  const chrome = requireChrome("properties-doors.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "properties-doors-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("properties-doors-harness.html", verdict);
  } finally {
    server.close();
  }
});
