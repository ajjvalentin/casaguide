...
test("V2-73 : le marqueur d'activité est distinct (act-pin)", async (t) => {
  const chrome = requireChrome("guide-activities.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "guide-activities-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("guide-activities-harness.html", verdict);
  } finally {
    server.close();
  }
});
