/* Carte filtrée en tête de catégorie du guide voyageur (V2-12e).

   Pilote le VRAI /frontend/guide/app.js dans Chrome headless (harnais
   guide-map-filter-harness.html), avec la vraie guide.css chargée et un mock
   Leaflet : entrée en mode filtré → carte visible sous les onglets + barre
   d'urgences masquée (rendu réel), marqueurs = sous-ensemble exact de la
   catégorie (+ logement), fitBounds appelé (zoom plafonné à 1 POI) ; retour →
...
test("carte filtrée V2-12e : carte visible + urgences masquées, marqueurs sous-ensemble, fitBounds, pas de doublon", async (t) => {
  const chrome = requireChrome("guide-map-filter.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "guide-map-filter-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("guide-map-filter-harness.html", verdict);
  } finally {
    server.close();
  }
});
