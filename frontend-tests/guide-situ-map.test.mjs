/* Carte de situation du logement (V2-53/53b) — recette navigateur RÉELLE.

   Pilote le VRAI /frontend/guide/app.js dans Chrome headless (harnais
   guide-situ-map-harness.html) avec la vraie guide.css et un mock Leaflet : la
   mini-carte a TOUTES ses interactions coupées, et le clic Leaflet (carte ET
   marqueur) bascule vers l'onglet « Autour de vous » RÉELLEMENT VISIBLE
   (offsetParent). Corrige la recette 10/09 où le tap ne basculait pas. Verdict lu
   dans le DOM dumpé (test ignoré si aucun Chrome).
...
test("carte de situation V2-53b : interactions coupées + tap (map & marqueur) bascule vers Autour visible", async (t) => {
  const chrome = requireChrome("guide-situ-map.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "guide-situ-map-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("guide-situ-map-harness.html", verdict);
  } finally {
    server.close();
  }
});
