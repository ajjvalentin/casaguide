/* « Premiers pas » (V2-31, volet 3b) — comportement dans Chrome headless.

   Pilote la vraie vue frontend/js/views/premierspas.js (harnais
   premierspas-harness.html) : les 7 étapes dans l'ordre avec leurs étiquettes,
   chaque CTA qui résout (logement courant / repli), les rubriques d'aide groupées
...
test("Premiers pas V2-31 : 7 étapes, CTAs, couverture des rubriques, repli, état vide", async (t) => {
  const chrome = requireChrome("premierspas.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "premierspas-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("premierspas-harness.html", verdict);
  } finally {
    server.close();
  }
});
