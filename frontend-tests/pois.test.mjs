/* Jour du marché (V2-33 volet 1) — écran de validation des lieux (back-office).

   Pilote le VRAI /frontend/js/views/pois.js dans Chrome headless (harnais
   pois-harness.html) : liste chronologique, badges plateforme, alerte de
...
test("lieux V2-33 : champ « Jour du marché » visible seulement pour la catégorie market", async (t) => {
  const chrome = requireChrome("pois.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "pois-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("pois-harness.html", verdict);
  } finally {
    server.close();
  }
});
