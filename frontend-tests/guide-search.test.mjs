/* Recherche dans le guide voyageur (V2-33 volet 2).

   Pilote le VRAI /frontend/guide/search.js dans Chrome headless (harnais
   guide-search-harness.html) : liste chronologique, badges plateforme, alerte de
   chevauchement, rotation avec fenêtre horaire, bouton « Compléter » d'un bloc,
   séjours annulés repliés, flux masqué, création d'un séjour direct. Verdict lu
   dans le DOM dumpé (test ignoré proprement si aucun Chrome).
...
test("guide V2-33 : recherche par mots-clés (faute de frappe, 2e langue, secret exclu, jamais vide)", async (t) => {
  const chrome = requireChrome("guide-search.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "guide-search-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("guide-search-harness.html", verdict);
  } finally {
    server.close();
  }
});
