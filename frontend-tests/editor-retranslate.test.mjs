/* Signal actif « traductions périmées » de l'éditeur (V2-41, pièce b).

   Pilote le VRAI /frontend/js/views/editor.js dans Chrome headless (harnais
   editor-retranslate-harness.html) : après une sauvegarde de section, si le badge
   de traductions est périmé, un bandeau non bloquant « Retraduire maintenant »
   apparaît ; l'action déclenche la traduction existante, puis le bandeau disparaît
...
test("V2-41 : sauvegarde → bandeau si périmé → « Retraduire maintenant » → badge à jour sans reload", async (t) => {
  const chrome = requireChrome("editor-retranslate.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "editor-retranslate-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("editor-retranslate-harness.html", verdict);
  } finally {
    server.close();
  }
});
