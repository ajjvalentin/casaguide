...
test("fenêtre « Envoyer le guide » : séjours à venir, lien /b/, langue, invitation", async (t) => {
  const chrome = requireChrome("send.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "send-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("send-harness.html", verdict);
  } finally {
    server.close();
  }
});
