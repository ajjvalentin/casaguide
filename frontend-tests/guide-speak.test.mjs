/* « Écouter le nom local » (V2-67) — guide voyageur.
   ...
   guide-speak-harness.html). Vérifie le GARDE-FOU (aucun bouton tant qu'aucune voix de
...
test("V2-67 : bouton « écouter » sur le nom local (garde-fou voix + prononciation)", async (t) => {
  const chrome = requireChrome("guide-speak.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "guide-speak-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("guide-speak-harness.html", verdict);
  } finally {
    server.close();
  }
});
