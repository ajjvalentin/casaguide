/* « Écouter le nom local » (V2-67) — guide voyageur.

   Pilote le VRAI /frontend/guide/app.js (initSpeak) dans Chrome headless (harnais
   guide-medallion-harness.html). Vérifie le GARDE-FOU (aucun bouton tant qu'aucune voix de
...
test("V2-72b : le médaillon pays est bâti dans la carte « Autour »", async (t) => {
  const chrome = requireChrome("guide-medallion.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "guide-medallion-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("guide-medallion-harness.html", verdict);
  } finally {
    server.close();
  }
});
