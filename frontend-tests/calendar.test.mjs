/* Calendrier des séjours (V2-23a).
   ...
   Pilote le VRAI /frontend/js/views/calendar.js dans Chrome headless (harnais
   calendar-harness.html) : liste chronologique, badges plateforme, alerte de
...
test("calendrier V2-23a : liste, badges, alerte, rotation, compléter, flux, création", async (t) => {
  const chrome = requireChrome("calendar.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "calendar-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("calendar-harness.html", verdict);
  } finally {
    server.close();
  }
});
