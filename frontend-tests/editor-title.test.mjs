/* Signal actif « traductions périmées » de l'éditeur (V2-41, pièce b).
   ...
   editor-title-harness.html) : après une sauvegarde de section, si le badge
...
test("V2-42 : champ « Titre de la rubrique » présent, PUT le porte, vidage → NULL", async (t) => {
  const chrome = requireChrome("editor-title.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "editor-title-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("editor-title-harness.html", verdict);
  } finally {
    server.close();
  }
});
