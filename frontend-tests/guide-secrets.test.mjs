/* Bloc secrets (wifi + boîte à clés) i18n côté client (V2-39) — guide voyageur.

   Pilote le VRAI /frontend/guide/app.js (initSecrets → fillWifi/fillKeybox) dans
   Chrome headless (harnais guide-secrets-harness.html). Guide en ANGLAIS : le bloc
   « Connexion Wifi » — rendu côté client car les secrets sont chiffrés — doit
   s'afficher intégralement en anglais, à partir des libellés servis par le SSR
   (data-secret-labels). Vérifie aussi le retour de copie localisé, l'absence de
...
test("V2-39 : bloc wifi/boîte à clés rendu côté client, intégralement dans la langue du guide", async (t) => {
  const chrome = requireChrome("guide-secrets.test.mjs");   // pas de navigateur → LÈVE (V2-76)
  if (!chrome) { t.skip("aucun navigateur — cf. bandeau final"); return; }   // hatch explicite
  const server = await startServer();
  try {
    const verdict = await runHarness(chrome, server.address().port, "guide-secrets-harness.html");
    assert.ok(verdict, "verdict du harnais introuvable dans le DOM dumpé");
    reportVerdict("guide-secrets-harness.html", verdict);
  } finally {
    server.close();
  }
});
